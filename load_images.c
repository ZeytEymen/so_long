...
/*   Updated: 2024/04/22 20:35:04 by ekarabud         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	take_images(t_mlx *map)
{
	...
	map->player = mlx_xpm_file_to_image(map->mlx_init, "./textures/bird.xpm",
			&w, &h);
	map->wall = mlx_xpm_file_to_image(map->mlx_init, "./textures/wall.xpm", &w,
			&h);
	map->collectible = mlx_xpm_file_to_image(map->mlx_init,
			"./textures/worm.xpm", &w, &h);
	map->grass = mlx_xpm_file_to_image(map->mlx_init, "./textures/grass.xpm",
			&w, &h);
	map->exit = mlx_xpm_file_to_image(map->mlx_init, "./textures/nest.xpm", &w,
			&h);
	if ((!map->player) || (!map->wall) || (!map->collectible) || (!map->grass)
		|| (!map->exit))
...
{
	mlx_put_image_to_window(map->mlx_init, map->mlx_window, image,
		x * 64, y * 64);
}
...
void	load_images(t_mlx *map, int x, int y)
{
	...
	{
		ft_printf("Unexpected character :%c\nExcepted : [0,1,C,E,P]\n",
			map->map_line[y][x]);
		abort_game(map);
	}
}
